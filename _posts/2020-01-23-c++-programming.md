...
- **lvalue** is ANY label that appears on the left of an assignment statement, variable name

- **rvalue** is ANYTHING to the right of an assignment statement, value assigned to the variable
...
- All statements get executed simultaneously when the name is called
![Functions-1](\assets\img\posts_data\c++\Functions1.png)
...
- Formal parameters accept values from the caller, destroyed when the functions complete
- Passing arguments to functions
- **Call by value** = copies the value of an argument into a parameter, changes made to the parameter do not affect the argument
- This is the default method for C++
- **Call by reference** = address of an argument is copied, changes made to the parameter when called this way **DO** affect the argument.
- Pass a pointer instead of the argument itself
- Parameters must be followed by & (ampersand) which means the variable is to be passed
- ![Functions-2](\assets\img\posts_data\c++\Functions2.png)
- Swap functions exchanges the values of two variables
- Modification in the swap functions **WILL** affect the variables passed